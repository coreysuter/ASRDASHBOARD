...
async function loadData(){
  // 1) If a global DATA already exists, use it.
  if (window.DATA && typeof window.DATA === 'object') return window.DATA;

  // 2) If index.html embedded JSON into <script id="data" type="application/json">, wait for it.
  const getEmbedded = () => {
  ...
  // Try immediately, then briefly wait (for the async fetch-inject script in index.html).
  let embedded = getEmbedded();
  if (embedded) return embedded;

  for (let i = 0; i < 60; i++){ // ~1s max at 16ms ticks
    await new Promise(r => requestAnimationFrame(r));
    embedded = getEmbedded();
    if (embedded) return embedded;
  }

  // 3) Fallback: fetch data.json directly (works on GitHub Pages / local server)
  try{
    const res = await fetch('data.json', { cache: 'no-store' });
    if (!res.ok) throw new Error('HTTP ' + res.status);
    return await res.json();
  }catch(e){
    throw new Error('Could not load data.json. Make sure it exists in the same folder and you are not using file://. Details: ' + (e.message || e));
  }
}
...
boot().then(() => {
  ...
  const app = document.getElementById('app');
  if(app){
    app.innerHTML = '<div class="panel"><div class="phead"><div class="h2">Data load error</div><div class="sub">Your site couldn\'t load <b>data.json</b>.</div></div><div class="list"><pre style="white-space:pre-wrap;color:var(--muted)">'+safe(e.stack||String(e))+'</pre></div></div>';
  }
});
